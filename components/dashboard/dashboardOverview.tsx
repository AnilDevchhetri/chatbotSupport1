...
const DashboardOverview = () => {
    ...
    return (
        <div className="p-5 md:p-6 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {setupSteps.map((step, i) => (
                        <Link key={i} href={step.href} className="block group">
                            <Card
                                className={cn(
                                    "border-white/5 bg-white/2 hover:bg-white/4 transition-colors h-full",
                                    step.complete
                                        ? "opacity-60"
                ...
                <div className='lg:col-span-2 space-y-8'>
                    <Card className='border-white/5 bg-[#0a0a0E]'>
                        ...
                        <CardContent className='grid grid-cols-3 gap-4'>
                            ...
                            <div className='p-3 rounded-lg bg-white/2 border border-white/5'>
                                <div className='flex items-center gap-2 mb-1'>
                                    <Text className='w-4 h-4 text-purple-400' />
                                    <span className='text-xs text-zinc-500 font-medium'>
                                        Manual Texts
                                    </span>
                                </div>
                    ...
                    <Card className='border-white/5 bg-[#0A0A0E] min-h-90'>
                        <CardHeader className='flex flex-row items-center justify-between pb-2'>
                            <div className='space-y-1'>
                                ...
                                <CardDescription>
                                    configure behavior diffenret topics
                                </CardDescription>
                            </div>
                            <Button size={"sm"}
                                className='h-8 gap-1 bg-white text-black hover:bg-zinc-200' asChild
                            >
                        ...
                        <CardContent className='p-0'>
                            <div className='divided-y divide-white/5'>
                                {
                                    sections.list.length === 0 ? (
                                        <div className='p-6 text-center text-sm text-zin-500'>
                                        ...
                                        </div>
                                        {
                                            sections?.list.map((section: any, i: number) => (
                                                <div key={i} className='grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 items-center hover:bg-white/2 transition-colors last:border-0 group'>
                                                    ...
                                                    <div className='col-span-3 text-sm font-medium text-zinc-200'>
